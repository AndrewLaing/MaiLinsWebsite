<!--
 * Filename:     detailsChanged.php
 * Author:       Andrew Laing
 * Email:        parisianconnections@gmail.com
 * Last updated: 31/05/2019.
 * Description:  The Details Changed page is used to confirm 
 *               to the user that their details have been changed to those 
 *               entered on the form in the changeAccountDetails.php page. 
-->
...
<?php 
    ...
    if(strlen($lname)==0 || strlen($fname)==0 || strlen($postcode)==0 || (strlen($phoneno)==0 && strlen($email)==0) ) {
      echo '<h1>ERROR</h1>';
    ...
    if ($stmt = $mysqli->prepare("CALL updateCustomerDetails(?,?,?,?,?,?,?,?)")) {
      $stmt->bind_param("isssssss",$customerID, $lname, $fname, $addr1, $addr2, $postcode, $phoneno, $email);
      
      if($stmt->execute()) {
        echo '<br /><br />';
        echo '<h1 class="indentedText">Success</h1><p class="indentedText">Your account details have been updated.</p>';
        echo '<br />';
        ...
        echo '<br /><br />';
        echo '<h1 class="indentedText">ERROR</h1><p class="indentedText">Unable to update account details.</p>';
        echo '<br />';
        echo '<p class="indentedText">You will be redirected back to the home page in 5 seconds</p>';
